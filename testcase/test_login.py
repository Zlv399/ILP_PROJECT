...
class TestLogin():
    ...
    @pytest.mark.parametrize('test_info',test_data['cases_login'],
                             ids=[item['casename'] for item in test_data['cases_login']])
    def test_login(self,test_info,browser):
        """
        包含所有情况参数化登录
        :param test_info: 登录参数
        ...
        login = LoginPage(browser)
        #参数化生成用例标题
        allure.dynamic.title(test_info['casename'])
        with allure.step('输入账号密码，操作登录'):
            login.get_login().login(test_info['username'],test_info['password'])
        with allure.step('获取登录结果'):
            expected = login.get_index_user_txt()
        assert test_info['expected'] == expected